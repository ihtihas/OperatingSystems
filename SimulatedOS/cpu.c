...
void fetch_instruction()
{
  ...
  else if (mret == mPFault)
  {
    ...
    set_interrupt(pFaultException);
      ...
      if (CPU.IRopcode != OPend && CPU.IRopcode != OPsleep && CPU.IRopcode != OPstore)
  {
    ...
    {
      CPU.exeStatus = ePFault;
      set_interrupt(pFaultException); //**Change**//
    }
...
void execute_instruction()
{
  ...
  case OPstore:
    // *** ADD CODE for the instruction
    //if ePfault, set status and interbit(see above)
